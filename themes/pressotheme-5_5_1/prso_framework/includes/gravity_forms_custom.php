...
function prso_theme_gform_field_content( $content, $field, $value, $lead_id, $form_id ) {
	...
	if( !is_admin() ) {
			...
			} elseif ( ($field['type'] === 'post_custom_field') || ($field['type'] === 'password') || ($field['type'] === 'radio') || ($field['type'] === 'select') || ($field['type'] === 'phone') || ($field['type'] === 'fileupload') ) {
...
function prso_theme_gform_get_address_field( $field, $value, $lead_id, $form_id ) {
    ...
    if(RG_CURRENT_VIEW == "entry"){
        ...
        $post_link = "";
        if(is_numeric($post_id) && RGFormsModel::is_post_field($field)){
